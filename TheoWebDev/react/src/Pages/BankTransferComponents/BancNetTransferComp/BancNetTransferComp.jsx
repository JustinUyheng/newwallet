...
import { Bank } from "../../LoginComponents/Icons";
...
function BancNetTransferComp() {
	...
	const handleTransfer = (e) => {
		e.preventDefault();
		const finalBankName =
			selectedBank === "Other" ? otherBankName : selectedBank;

		if (
			!accountNumber ||
			!finalBankName ||
			!amount ||
			finalBankName === "Select Bank"
		) {
		...
		setMessage(
			`Initiating BancNet transfer of ₱${amount} to ${finalBankName} account ${accountNumber}`
		);
		setAccountNumber("");
		setAmount("");
	...
	return (
		<div className="transfer-section bancnet-section">
			<div className="section-header bancnet-header">
				<img src={Bank} alt="BancNet Logo" className="logo" />
				<h2>Bank Transfer</h2>
			</div>
