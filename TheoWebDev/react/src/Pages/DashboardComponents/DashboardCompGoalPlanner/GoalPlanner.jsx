...
function GoalPlanner() {
	// No longer accepts props, fetches its own data
	...
	const fetchData = useCallback(async () => {
		...
		try {
			...
			// Fetch current money
			const moneyResponse = await axiosClient.get("/user/money", {
				headers: { Authorization: `Bearer ${token}` },
			});
	...
	const totalTargetBalance = goals.reduce(
	...
	return (
		<div className={styles.goalPlannerCard}>
			...
			<div className={styles.goalDetail}>
				...
				{/* Updated class name */}
				<div className={styles.goalDetailList}>
					{" "}
					{/* NEW: Added this div and class */}
					{goals && goals.length > 0 ? (
						goals.map((item) => (
							<div className={styles.goalDetailItem} key={item.id}>
								{" "}
								{/* Updated class name & key */}
								<span>{item.purpose}</span>{" "}
								{/* Changed from item.name to item.purpose */}
								<span>
									₱
