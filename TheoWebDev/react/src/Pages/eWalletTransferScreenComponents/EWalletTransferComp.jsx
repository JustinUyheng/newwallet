...
function EWalletTransferComp() {
	...
	const handleSubmit = async (e) => {
		...
		const recipientIdentifier =
			contactNumberRef.current.value || emailRef.current.value;
		...
		if (!payload.recipient_identifier) {
			setErrors({
				general: [
					"Please provide either an email or a contact number for the recipient.",
				],
			});
			return;
		}
		...
		<div className={styles.dashboardRoot}>
			<div className={styles.authWrapper}>
				...
				<div className={styles.ewalletFormContainer}>
					...
					{message && (
						<p className={styles.ewalletFeedbackMessage}>{message}</p>
					)}
				</div>
			</div>
		</div>
